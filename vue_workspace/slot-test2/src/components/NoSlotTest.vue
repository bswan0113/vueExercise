<template>
    <div>
        <h3>당신이 경험한 프론트 엔드 기술은?(첫번째 : 슬롯사용 X)</h3>
        <CheckBox1 v-for="item in items" :key="item.id" :id="item.id" :label="item.label" :checked="item.checked" @check-changed ="CheckBoxChanged"></CheckBox1>
    </div>
</template>
